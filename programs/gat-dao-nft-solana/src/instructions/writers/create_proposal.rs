...
use crate::{
    helpers::{
        enums::VoteBy, errors::ErrorCode, events::*, Utils, DAO_INFO_ACCOUNT_PREFIX,
        GOVERNANCE_DAO_INFO_ACCOUNT_PREFIX, PROPOSAL_INFO_ACCOUNT_PREFIX,
        WRITER_CREATE_PROPOSAL_ROLE, WRITER_DAO_INFO_ACCOUNT_PREFIX,
    },
    schemas::{
        DaoInfo, DaoInfoAccountBehavior, GovernanceInfo, ProposalInfo, VoteInfo, WriterInfo,
...
pub fn create_proposal(
    ...
    if !writer_info.has_role(ctx.accounts.creator.key(), WRITER_CREATE_PROPOSAL_ROLE)
        || ctx.accounts.creator.key() == ctx.accounts.writer.key()
    {
        return Err(error!(ErrorCode::PermissionRequired));
    }
    if !writer_info.is_valid_proposal_id(proposal_id)
